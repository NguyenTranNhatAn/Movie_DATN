...
// // Giá ghế ví dụ
// const SEAT_PRICE = 104909;

// // Component ghế được memo hóa
// const Seat = memo(({ seatId, isSelected, onSeatPress, isMinimap }) => (
//   <TouchableOpacity
//     key={ seatId }
//     style={ [
//       styles.seat,
...
//     disabled={ isMinimap } // Không cho phép nhấn chọn ghế trong minimap
//   >
...
//   const renderSeats = useCallback(
//     (isMinimap = false) => {
//       const seatString = 'TTTTPTTUPP_______/VVPUVVVVVV_______/DDUPUDDDDD_______/';
//       const rows = seatString.split('/');
//       const seats = [];
...
//           const char = rows[i][j];
//           if (char !== '_') {
//             const seatId = `${String.fromCharCode(65 + i)}${j + 1}`; // Chuyển đổi thành ký hiệu A1, B2, ...
//             const isSelected = selectedSeats.includes(seatId);
//             row.push(
//               <Seat
//                 key={ seatId }
...
const SeatSelectionScreen = () => {
  ...
  const socket = io('https://60d8-171-252-189-233.ngrok-free.app');
  ...
  const loadSeatMap = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://60d8-171-252-189-233.ngrok-free.app/showtimes/${showtimeId}`);
  ...
  const handleBook = async () => {
    ...
    const bookingData = {
      ...
      cancelUrl: 'myapp://home'  // URL khi người dùng hủy thanh toán
    };
    //
    try {
      const response = await fetch('https://60d8-171-252-189-233.ngrok-free.app/order/create', {
        method: 'POST',
        headers: {
