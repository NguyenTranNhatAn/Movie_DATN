  ...
  const handleSeatPress = useCallback((seatId, rowIndex, colIndex, seatType) => {
    ...
    setSelectedSeats((prevSeats) => {
      ...
      if (isSeatSelected) {
        ...
        console.log('in ra cái ghế chọn', selectedSeats);
      }
   ...
   const handleSeatPress = useCallback((seatId, rowIndex, colIndex, seatType) => {
     ...
     setSelectedSeats((prevSeats) => {
       ...
       if (isSeatSelected) {
         ...
         updatedSeats = [
           ...prevSeats,
           { seatId, rowIndex, colIndex, price: seatPrice, time: currentTime },
         ];
         socket.emit('select_seat', { showtimeId, row: rowIndex, col: colIndex, userId, time: currentTime });
         setSeatMap((prevMap) =>
           prevMap.map((rowSeats, rIndex) =>
             rIndex === rowIndex
               ? rowSeats.map((seat, cIndex) => (cIndex === colIndex ? 'P' : seat)) // 'P' = Selected
               : rowSeats
           )
         );
       }
     ...
     socket.once('seat_response', (response) => {
       if (!response.success) {
         Alert.alert('Thông báo', response.message || 'Ghế này đã được chọn bởi người khác.');
         setSeatMap((prevMap) =>
           prevMap.map((rowSeats, rIndex) =>
             rIndex === rowIndex
               ? rowSeats.map((seat, cIndex) => (cIndex === colIndex ? seatType : seat))
               : rowSeats
           )
         );
       }
